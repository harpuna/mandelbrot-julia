...
class Julia:
    ...
    def __init__(self, config):
        ...
        self.config_string = (
            f"xs_{self.x_start}_ys_{self.y_start}_w_{self.width}_h_{self.height}_r_{self.init_r:.5f}_d_{self.density}_t_{self.threshold}"
        )
    ...
    def animate(self, fi):
        ...
        cx = self.init_r * np.cos(self.a[fi])
        cy = self.init_r * np.sin(self.a[fi])
    ...
    def generate(self):
        ...
        anim.save(f"julia_{self.config_string}.gif", writer='imagemagick')
...
Julia(
    {
        ...
        "init_r": 1.7885,   # we represent c as c = r*cos(a) + i*r*sin(a) = r*e^{i*a}
        "density": 1000,     # more detail (200 is decent)
        "threshold": 500,    # more detail (20 is decent)
